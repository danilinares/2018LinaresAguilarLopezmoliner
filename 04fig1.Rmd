...
# sym_par_s_vs_d_best_abs_boot <- sym_par_s_vs_d_best_abs %>% 
#   group_by(parn) %>% 
#   nest() %>% 
#   mutate(b = map(data, ~boot(.$abs_par,  
#                              statistic = 
#                                function(data, i){
#                                  d <- data[i]
#                                  mean(d)  
...
#          y = map_dbl(b, "t0"),
#          samples = map(b, "t"),
#          ci = map(b, ~boot.ci(., type = "perc")),
#          ymin = map_dbl(ci, ~.$percent[1, 4]),
#          ymax = map_dbl(ci, ~.$percent[1, 5]))
# 
# sym_par_s_vs_d_best_abs_boot_samples <- sym_par_s_vs_d_best_abs_boot %>% 
#   select(parn, samples) %>% 
#   unnest(samples) %>% 
#   group_by(parn) %>% 
...
#   group_by(row) %>% 
#   spread(parn, samples) %>% 
#   rename(decision = `Decisional\nbias`, sensory = `Sensory\nbias`) %>% 
#   mutate(dif = sensory - decision) %>% 
#   ungroup() %>% 
#   summarise(ymin = quantile(dif, .025),
#             ymax = quantile(dif, .975))
# 
# 
