...
p_corr <- ggplot(data = sensory_bias %>% 
  ...
  geom_point(aes(x = asym + 0.03, y = sym - 0.03, color = best_sym,
                 shape = "Asymmetric"), 
             size = 2, alpha = .75) +
  geom_point(aes(x = asym - 0.03, y = sym + 0.03, color = best_asym,
                 shape = "Symmetric"), 
              size = 2, alpha = .75) +
  ...
  labs(x = expression(paste("Sensory bias ", bold("asymmetric"), " task (deg)")),
       y = expression(paste("Sensory bias ", bold("symmetric"), " task (deg)")),
       shape = "Task", 
